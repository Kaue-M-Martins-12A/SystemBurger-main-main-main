...
exports.getAdminDashboard = async (req, res) => {
    try {
        ...
        let dateLimit = "";
        if (range === 'week') dateLimit = "datetime('now', '-7 days', 'localtime')";
        else if (range === 'month') dateLimit = "datetime('now', '-30 days', 'localtime')";
        else if (range === 'year') dateLimit = "datetime('now', '-1 year', 'localtime')";
        ...
        if (range === 'week') {
            salesQuery = `
                SELECT strftime('%d/%m', datetime(created_at, 'localtime')) as label, SUM(total) as value 
                FROM orders 
                ${whereClause}
            ...
            salesQuery = `
                SELECT ('Semana ' || ((CAST(strftime('%d', datetime(created_at, 'localtime')) AS INTEGER) - 1) / 7 + 1)) as label, SUM(total) as value 
                FROM orders 
                ${whereClause}
                GROUP BY label ORDER BY MIN(created_at) ASC
            `;
            ...
            salesQuery = `
                SELECT strftime('%m/%Y', datetime(created_at, 'localtime')) as label, SUM(total) as value 
                FROM orders 
                ${whereClause}
                GROUP BY label ORDER BY MIN(created_at) ASC
            `;
        ...
        // Para a view semanal, gerar todos os 7 dias com valores preenchidos
        if (range === 'week') {
            // Construir mapa de label -> valor a partir dos dados do banco
            const rawMap = {};
            salesData.forEach(r => { rawMap[r.label] = parseFloat(r.value) || 0; });
